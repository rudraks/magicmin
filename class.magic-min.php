...
class Minifier {
    ...
    private function merge_images( $source_file, $contents )
    {
        ...
        {
        	//$this->messages[]['Minifier Log: merge_images'] = json_encode($this->matches);
            $this->find = array();
            $this->replace = array();
            ...
            foreach( $this->matches as $this->graphic )
            {
                ...
                {
                	$this->messages[]['Minifier Log: merge_images'] = "File is remote". $this->graphic[2];
                ...
                elseif( file_exists( $this->directory . $this->graphic[2] ) )
                {
                	...
                	$this->messages[]['Minifier Log: merge_images'] = "File DOES exist locally, get the contents". $this->graphic[2];
                	...
                	$this->messages[]['Minifier Log: merge_images'] = "File DOES not exist locally, get the contents". $this->graphic[2];
    ...
    private function make_min( $src_file, $new_file, $do_minify = true )
    {
        ...
        {
        	//Browser::error('singel_file'.$new_file);
        ...
        {
        	//Browser::error('multipla'.$new_file);
        ...
        //Browser::error('Log any error messages from the new file creation'.$new_file);
        if( !$this->handle )
        {
    ...
    public function minify( $src_file, $file = '', $version = '' )
    {
        ...
        //Browser::info("minifying file::".$src_file);	
        if( empty( $file ) )
        {
            //Get the pathinfo
        ...
        {
        	//Browser::error('//No EXITS',$orig_path,$src_file,file_exists( $src_file ));
            //Make the cached version
            $this->output_file = $this->make_min( $src_file, $file );
